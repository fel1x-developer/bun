...
pub fn DeleteFileBun(sub_path_w: []const u16, options: DeleteFileOptions) JSC.Maybe(void) {
    ...
    if (JSC.Maybe(void).errnoSys(rc, .open)) |err| {
        return err;
    }
    ...
    if (JSC.Maybe(void).errnoSys(rc, .NtSetInformationFile)) |err| {
        return err;
    }
